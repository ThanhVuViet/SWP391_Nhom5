    ...
    <form id="quizForm" action="SubmitQuiz" method="post">
        ...
        <div class="question" id="question_<%= i %>">
            ...
            <div class="alternatives">
                ...
                %>
                <input type="checkbox" name="answer_<%= i %>" value="<%= choice.getChoiceId() %>" onclick="highlightQuestion(<%= i %>, this)" <%= request.getParameterValues("answer_" + i) != null && java.util.Arrays.asList(request.getParameterValues("answer_" + i)).contains(String.valueOf(choice.getChoiceId())) ? "checked" : "" %>> <%= choice.getChoiceText() %><br>
                <%
                    }
                %>
            </div>
...
<script>
    ...
    function highlightQuestion(index, input) {
        ...
        if (!selectedAnswers['answer_' + index]) {
            selectedAnswers['answer_' + index] = [];
        }

        if (input.checked) {
            selectedAnswers['answer_' + index].push(input.value);
        } else {
            const indexToRemove = selectedAnswers['answer_' + index].indexOf(input.value);
            if (indexToRemove > -1) {
                selectedAnswers['answer_' + index].splice(indexToRemove, 1);
            }
        }
    ...
    window.onload = function() {
        ...
        for (let key in selectedAnswers) {
            ...
            selectedAnswers[key].forEach(value => {
                document.querySelector(`input[name="${key}"][value="${value}"]`).checked = true;
                highlightQuestion(index, document.querySelector(`input[name="${key}"][value="${value}"]`));
            });
        }
    };
</script>
