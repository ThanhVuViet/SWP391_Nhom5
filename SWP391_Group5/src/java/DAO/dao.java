...
public class dao extends MyDAO {
    ...
    public Expert getExpertByUserId(int id) {
        String query = "SELECT * FROM Experts e JOIN Users u ON e.user_id = u.user_id WHERE u.user_id = ?";
        Expert expert = null;
        try {
            ps = con.prepareStatement(query);
            ps.setInt(1, id);
    ...
    public boolean registerUser(Users user) {
        String query = "INSERT INTO Users (role_id, username, password, full_name, email, birth_date, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)";
        try {
            ps = con.prepareStatement(query);
    ...
    public boolean registerPendingUser(Users user, String description, String certification) {
        String query = "INSERT INTO PendingRegistrations (username, email, password, full_name, birth_date, role_id, created_at, description, certification) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
        try {
            ps = con.prepareStatement(query);
            ps.setString(1, user.getUsername());
            ps.setString(2, user.getEmail());
            ps.setString(3, user.getPassword());
            ps.setString(4, user.getFullName());  // Cập nhật full name
            ps.setDate(5, new java.sql.Date(user.getBirthDate().getTime()));
            ps.setInt(6, user.getRoleId());
            ps.setTimestamp(7, new Timestamp(System.currentTimeMillis()));
            ps.setString(8, description);  // Thêm description
            ps.setString(9, certification);  // Thêm certification
            int rowsAffected = ps.executeUpdate();
            return rowsAffected > 0;
        } catch (Exception e) {
            e.printStackTrace(); // Handle exceptions appropriately
    ...
    public List<Users> getPendingExperts() {
        List<Users> pendingExpertList = new ArrayList<>();
        String query = "SELECT * FROM PendingRegistrations";
        try {
            ps = con.prepareStatement(query);
            rs = ps.executeQuery();
            while (rs.next()) {
                ...
                pendingExpertList.add(user);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
        return pendingExpertList;
    }

    public boolean approveExpert(Users user) {
        String query = "INSERT INTO Users (username, email, password, full_name, birth_date, role_id, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)";
        try {
            ps = con.prepareStatement(query);
            ps.setString(1, user.getUsername());
            ps.setString(2, user.getEmail());
    ...
    public Users getPendingExpertById(int userId) {
        String query = "SELECT * FROM PendingRegistrations WHERE id = ?";
        try {
            ps = con.prepareStatement(query);
            ps.setInt(1, userId);
            rs = ps.executeQuery();
    ...
    public boolean deletePendingExpert(int userId) {
        String query = "DELETE FROM PendingRegistrations WHERE id = ?";
        try {
            ps = con.prepareStatement(query);
            ps.setInt(1, userId);
            int rowsAffected = ps.executeUpdate();
    ...
    public boolean addExpert(Users user) {
        String sql = "INSERT INTO Experts (user_id) VALUES (?)";
        try {
    ...
    public int getUserIdByUsername(String username) {
        ...
        return -1; // User ID not found or exception occurred
    }
    ...
    public List<Course> getCartCourses(int userId) {
        ...
        String sql = "SELECT c.course_id, c.title, c.price FROM Cart ct JOIN Courses c ON ct.course_id = c.course_id WHERE ct.user_id = ?";
        try {
            ps = con.prepareStatement(sql);
            ps.setInt(1, userId);
            rs = ps.executeQuery();
    ...
    public void deleteCourseFromCart(int userId, int courseId) {
    ...
    public List<Feedback> getFeedbackByCourseId(int courseId) {
        List<Feedback> feedbackList = new ArrayList<>();
        String sql = "SELECT f.feedback_id, f.user_id, f.rating, f.comment, f.created_at "
                + "FROM Feedback f JOIN Users u ON f.user_id = u.user_id "
                + "WHERE f.course_id = ?";

        try {
            ps = con.prepareStatement(sql); // Use PreparedStatement here
            ps.setInt(1, courseId);
            rs = ps.executeQuery(); // Use ResultSet here
            while (rs.next()) {
                Feedback feedback = new Feedback();
    ...
    public PendingExpert getObjPendingExpertById(int id) {
        PendingExpert pendingExpert = null;
        String sql = "SELECT * FROM PendingRegistrations WHERE id = ?";
        ...
        try {
            ...
            if (rs.next()) {
                pendingExpert = new PendingExpert();
                pendingExpert.setId(rs.getInt("id"));
                pendingExpert.setUsername(rs.getString("username"));
                pendingExpert.setEmail(rs.getString("email"));
                pendingExpert.setPassword(rs.getString("password"));
                pendingExpert.setFullName(rs.getString("full_name"));
                pendingExpert.setBirthDate(rs.getDate("birth_date"));
                pendingExpert.setRoleId(rs.getInt("role_id"));
                pendingExpert.setCreatedAt(rs.getTimestamp("created_at"));
                pendingExpert.setDescription(rs.getString("description"));
                pendingExpert.setCertification(rs.getString("certification"));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public List<PendingExpert> getAllPendingExperts() {
        List<PendingExpert> pendingExperts = new ArrayList<>();
        String sql = "SELECT * FROM PendingRegistrations";
        ...
        try {
            ...
            while (rs.next()) {
                PendingExpert pendingExpert = new PendingExpert();
                pendingExpert.setId(rs.getInt("id"));
                pendingExpert.setUsername(rs.getString("username"));
                pendingExpert.setEmail(rs.getString("email"));
                pendingExpert.setPassword(rs.getString("password"));
                ...
                pendingExpert.setDescription(rs.getString("description"));
                pendingExpert.setCertification(rs.getString("certification"));
                pendingExperts.add(pendingExpert);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public List<UserProgress> getProgressByUserId(int userId) {
        ...
        String sql = "SELECT * FROM CourseProgress WHERE user_id = ?";
        ...
        try {
            ...
            while (rs.next()) {
                ...
                progress.setGrade(rs.getInt("grade"));
                progress.setProgress(rs.getDouble("progress"));
                progress.setCompletedExamIds(rs.getString("completed_exam_ids"));
    ...
    public void updateUser(Users user) {
        String sql = "UPDATE Users SET email = ?, full_name = ?, phone_number = ?, address = ?, birth_date = ? WHERE user_id = ?";
    ...
    public void addFeedback(Feedback feedback) {
        String sql = "INSERT INTO Feedback (course_id, user_id, comment, rating, created_at) VALUES (?, ?, ?, ?, ?)";
        try {
            ps = con.prepareStatement(sql);
            ...
            ps.setDate(5, new java.sql.Date(feedback.getCreatedAt().getTime())); // Correct conversion
            ps.executeUpdate();
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public void addExamChoice(ExamChoice choice) {
        String sql = "INSERT INTO ExamChoices (choice_text, is_correct, question_id) VALUES (?, ?, ?)";
        try {
            ps = con.prepareStatement(sql);
    ...
    public void updateExamChoice(ExamChoice choice) {
        String sql = "UPDATE ExamChoice SET choice_text = ?, is_correct = ? WHERE choice_id = ?";
        try {
            ps = con.prepareStatement(sql);
            ps.setString(1, choice.getChoiceText());
            ps.setBoolean(2, choice.isCorrect());
    ...
    public void deleteExamChoice(int choiceId) {
        String sql = "DELETE FROM ExamChoices WHERE choice_id = ?";
        try {
            ps = con.prepareStatement(sql);
    ...
    public void addExamQuestion(ExamQuestion question) {
        String sql = "INSERT INTO ExamQuestions (question_text, exam_id) VALUES (?, ?)";
        try {
            ps = con.prepareStatement(sql);
    ...
    public void updateExamQuestion(ExamQuestion question) {
        String sql = "UPDATE ExamQuestions SET question_text = ? WHERE question_id = ?";
        try {
            ps = con.prepareStatement(sql);
            ps.setString(1, question.getQuestionText());
    ...
    public void deleteExamQuestion(int questionId) {
        String sql = "DELETE FROM ExamQuestions WHERE question_id = ?";
        try {
            ps = con.prepareStatement(sql);
            ps.setInt(1, questionId);
            ps.executeUpdate();
       ...
       public void deleteChoicesByQuestionId(int questionId) {
        String sql = "DELETE FROM ExamChoices WHERE question_id = ?";
    ...
    public void updatePassword(String username, String newPassword) {
      ...
      public void updateUserProfile(int userId, String fullName, String email, String phoneNumber, String address, String birthDate) {
        String sql = "UPDATE Users SET full_name = ?, email = ?, phone_number = ?, address = ?, birth_date = ? WHERE user_id = ?";
        try {
            ps = con.prepareStatement(sql);
    ...
    // Method to update expert profile
    public void updateExpertProfile(int userId, String description, String certification) {
        String sql = "UPDATE Experts SET description = ?, certification = ? WHERE user_id = ?";
        try {
            ps = con.prepareStatement(sql);
            ps.setString(1, description);
